 ...
 */
public interface MentionApprendreSurRepository extends PagingAndSortingRepository<MentionApprendreSur, Long> {

	@Query("MATCH (Collaborateur { akCollaborateur: {akCollaborateur} })-[MENTION_APPRENDRE_SUR]-(Competence { akCompetence: {akCompetence} }) RETURN *")
	List<MentionApprendreSur> findAkCollaborateurAndAkCompetence(@Param("akCollaborateur") String akCollaborateur, @Param("akCompetence") String akCompetence);
	
	@Query("RETURN EXISTS((:Collaborateur { akCollaborateur: {akCollaborateur} })-[:MENTION_APPRENDRE_SUR]-(:Competence { akCompetence: {akCompetence}}))")
